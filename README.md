...
at least $5. 2D Mockups and video are priced by what
they cost to produce, not at the flat render rate. A new
account gets 500 credits once and no card is required to spend them, but until a
card is verified its renders are watermarked and capped at 1,024 px, it can keep 5
PSD templates, and one render runs at a time. Funding the $5 minimum lifts every one
of those: the watermark and the width cap come off, the stored-template limit goes
to 150, and renders run 25 at a time alongside 10 concurrent uploads. There is no
